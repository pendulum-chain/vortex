...
export function usePrepareTransactions() {
  ...
  return useMemo(
    () =>
      async (state: OfframpingState, context: ExecutionContext): Promise<OfframpingState> => {
        if (!pendulumNode) {
    ...
    [pendulumNode, prepareSpacewalkRedeemTransaction, prepareNablaApproveTransaction, prepareNablaSwapTransaction],
  );
}
