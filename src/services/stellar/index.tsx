...
export async function setUpAccountAndOperations(
  ...
  const {offrampingTransaction, mergeAccountTransaction} = await createOfframpAndMergeTransaction(sep24Result, ephemeralKeys, ephemeralAccount);
...
async function setupStellarAccount(
  ...
  try{
    ...
    console.error(error);
    renderEvent(
      `Could not create the account creation transaction. ${error}`,
      EventStatus.Error,
    );
  ...
  try {
    ...
    renderEvent(
      `Could not submit the account creation transaction. ${JSON.stringify(horizonError.response.data.extras.result_codes)}`,
      EventStatus.Error,
    );
...
async function createOfframpAndMergeTransaction(sep24Result: Sep24Result, ephemeralKeys: Keypair, ephemeralAccount:Account ) {
