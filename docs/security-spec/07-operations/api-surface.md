...
17. **Ramp cancellation MUST require ownership and remain unsigned-only** — `POST /v1/ramp/cancel` requires user-or-partner authentication and `assertRampOwnership`. The service locks the ramp row and changes only unsigned `initial` ramps (`presigned_txs IS NULL`) to `timedOut`; an already-timed-out retry is idempotent. Signed ramps may have disclosed payment instructions, so cancellation returns `409` and the ramp remains resumable until server-side expiry.
...
| **Cross-user or payment-pending ramp cancellation** — A caller targets another user's ramp or abandons one after payment instructions were exposed | The cancellation route requires authenticated ownership; a row-locked service check accepts only unsigned `initial` ramps (or idempotent `timedOut`) and rejects signed or later ramps with `409`. |
