...
17. **Ramp cancellation MUST require ownership and remain phase-limited** — `POST /v1/ramp/cancel` requires user-or-partner authentication and `assertRampOwnership`. The service locks the ramp row and changes only `initial` ramps to `timedOut`; an already timed-out retry is idempotent and every other phase returns `409`. Cancellation remains available during maintenance so an owner can abandon an unstarted ramp without initiating new money movement.
...
| **Cross-user or mid-flow ramp cancellation** — A caller targets another user's ramp or attempts to stop one after processing begins | The cancellation route requires authenticated ownership; a row-locked service check accepts only `initial` (or idempotent `timedOut`) and rejects later phases with `409`. |
