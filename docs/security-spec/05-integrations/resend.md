...
2. **Transactional notifications** - Emails the API sends about account events: completed ramps, settled verification, and organization membership invitations. These go through the Resend HTTPS API from `apps/api`. Organization invites retain the internal `managed_profile_membership_invitation` discriminator and producer/template filenames.
