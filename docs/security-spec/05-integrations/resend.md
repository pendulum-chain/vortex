...
16. **Membership invitation links MUST use a trusted origin and minimal payload.** The API builds links from `DASHBOARD_PUBLIC_URL` plus the invitation UUID. Request headers and request body cannot select the origin. The template may identify the invited role and child display label but must not contain API secrets, bearer tokens, provider identity, financial data, or internal membership IDs other than the invitation locator.
...
- [ ] Migration `062` creates the base queue and migration 069 enforces exactly one recipient source plus the direct-email type allowlist
