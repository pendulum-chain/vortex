...
| **Long-lived on-ramp DOS** | Attacker creates many on-ramps to occupy ephemeral accounts for 24h | No per-user concurrent-ramp limit is currently enforced; this risk remains deferred under F-024. Generic IP rate limiting and user-funded ephemerals only partially bound the blast radius. |
