...
| **Unverified EUR ramp / KYC bypass** | A registered-but-not-KYC'd profile (or one whose Mykobo review is pending/rejected) registers an EUR ramp and receives SEPA payment instructions | Registration refreshes the Mykobo KYC mirror live and rejects with `400` unless `MykoboCustomer.status === APPROVED`; the Mykobo intent is created only after the gate passes. |
...
- [ ] EUR register (on-ramp and off-ramp) derives the Mykobo email from `profiles.email` via `resolveMykoboCustomerForUser`, rejects a non-matching client-supplied email with `400`, and requires `MykoboCustomer.status === APPROVED` before creating the Mykobo intent
