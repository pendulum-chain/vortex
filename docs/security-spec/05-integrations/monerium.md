...
quoted post-fee EURe from the owner, swaps it through the pinned Polygon Uniswap V3 EURe → USDC.e → USDC path,
...
native USDC `0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359`, bridged USDC.e
`0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174`, the 3000-fee EURe/USDC.e pool
`0x7d4324293304797cB662C6EA1B904B6AF2b485F5`, the 100-fee USDC.e/USDC pool
`0xD36ec33c8bed5a9F7B6630855f1533455b98a418`, factory
...
18. The Polygon conversion MUST verify each pinned pool's tokens, fee, and factory and verify that the pinned factory, router, and quoter resolve to that deployment before quoting or execution. It MUST quote and execute exact-input EURe-to-USDC only, through the pinned packed path, approve only the exact input, bind the swap recipient to the ephemeral, enforce the standard AMM hard minimum and soft execution threshold, validate both raw signed transactions against their unsigned blueprints and route semantics, verify successful receipts, and reconcile the post-swap allowance and output balance. When the swap's deterministic transaction hash already has a successful receipt, execution MUST skip the pre-broadcast deadline, deployment, balance, allowance, and quote checks (the mined swap has consumed the allowance), MUST settle the financial-operation row from that same receipt so a `submitted` row becomes `confirmed` without a second read that could fail transiently or a resend, and then proceed to that post-swap reconciliation, so a crash between settlement and phase completion resumes instead of pausing. Polygon USDC fee distribution and post-swap subsidy MUST use the existing configured fee recipients and EVM funding account respectively; neither may substitute the Monerium owner or ephemeral as a treasury destination.
...
21. `POST /v1/monerium/wallet` MUST verify the EOA signature over the fixed ownership message server-side before any provider call, MUST reject addresses with deployed code, MUST select the same legal profile type as the status read, MUST link through the app that can read that profile, MUST request its single IBAN only when it has none, and MUST hold the profile advisory lock through the list reads, the link, and the IBAN request so it serialises with IBAN moves and issue registration on that profile. `POST /v1/monerium/iban/move` MUST hold the same profile and current-owner advisory locks through the active-ramp check and provider mutation, and MUST reject with a public `409` while a live Monerium ramp still waits on the IBAN's current wallet, because the mint executor watches only the registered owner. Status reads (`GET /v1/monerium/status`, `GET /v1/onboarding/status`) MUST NOT mutate provider state; they report readiness for that legal type from the same list reads registration uses. OAuth-user API reads returning `401` at any point MUST surface `MONERIUM_REAUTHENTICATION_REQUIRED` so clients can reconnect.
...
- [x] Polygon conversion verifies both pinned Uniswap V3 pools, quotes exact input over the packed path, prepares exact approval and `exactInput` transactions, validates their signed semantics, and reconciles allowance, receipt, and output thresholds.
