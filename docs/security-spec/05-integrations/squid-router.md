...
| **Insufficient Axelar gas after payment** | Stuck monitor sends at most one supplemental `addNativeGas` top-up: a `"pending"` sentinel is persisted to `squidRouterExtraGasTxHash` before the broadcast and reconciled to the tx hash after, so a crash between send and persist cannot re-pay. Skipped in the iteration that made the initial payment (the fetched status predates it). Excess gas is refunded to the funding wallet by the gas service. |
