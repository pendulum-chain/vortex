...
| **Insufficient Axelar gas after payment** | Stuck monitor sends at most one supplemental `addNativeGas` top-up, guarded by the persisted `squidRouterExtraGasTxHash` and skipped in the iteration that made the initial payment (the fetched status predates it). Excess gas is refunded to the funding wallet by the gas service. |
