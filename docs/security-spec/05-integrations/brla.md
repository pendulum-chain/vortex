...
**Temporary disablement:** BRL↔AssetHub on/off-ramps are disabled while the new BRL rail runs on Base. The quote engine does not return those quotes. Both USDC topologies are nevertheless represented by the block catalog so persisted quotes, transaction preparation, and recovery remain deterministic; their legacy route assemblers are not reachable. Active BRL corridors are BRL↔supported EVM destinations via Base.
...
5. `distributeFees` (Multicall3 batch on Base, see `fee-integrity.md`) → `subsidizePostSwap` (if needed).
...
| **Disabled AssetHub corridor accidentally re-enabled** | A developer mistakes either cataloged recovery flow for an eligible production corridor | Quote eligibility rejects both BRL→AssetHub and AssetHub→BRL before simulation. Catalog and transaction parity tests may resolve the flows directly, but any successful public quote remains a regression until intentional re-enablement. |
...
- [x] BRL Base output topology is token-specific. **PASS** — block catalog resolution maps USDC to the no-Squid flow, BRLA to the direct bypass, and USDT/ETH/AXLUSDC/EURC to the one-phase same-chain Squid flow; parity tests enforce Base transaction construction and contiguous destination nonce ordering.
