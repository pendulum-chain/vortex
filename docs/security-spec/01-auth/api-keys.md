...
7. **Secret-key lookup MUST be bounded by a per-key identifier** — validation looks up by the 16-char lookup prefix (62^8 identifier space), so verification cost is O(1) in the number of active keys. Only when that lookup finds nothing does the legacy fallback scan rows stored under the constant 8-char prefix with bcrypt comparison; each legacy row leaves that pool permanently on its first successful validation (lazy upgrade, possible because the plaintext is in hand at auth time). The self-serve creation cap (invariant 17) bounds abuse of what remains of the legacy pool.
