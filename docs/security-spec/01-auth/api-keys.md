...
18. **Public and secret records in a new pair MUST share a `credential_id`** — Pair relationships must not be inferred from names. Paired self-serve revocation verifies equal credential IDs and updates both records in one transaction. Legacy records without a credential ID retain the name/type compatibility check; ambiguous records are never automatically paired.
