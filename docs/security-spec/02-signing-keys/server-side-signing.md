...
2. **Legacy-named EVM compatibility key** — `MOONBEAM_EXECUTOR_PRIVATE_KEY`: retained as a configuration fallback for general EVM account derivation and local status-address output. Moonbeam transaction execution, subsidization, and XCM are runtime-disabled.
...
6. **Missing mandatory keys MUST prevent server startup** — If `PENDULUM_FUNDING_SEED` or the currently required legacy-named `MOONBEAM_EXECUTOR_PRIVATE_KEY` compatibility fallback are absent, startup validation fails. This requirement reflects general EVM configuration compatibility, not active Moonbeam execution.
...
| **Executor key abuse** | Attacker with `MOONBEAM_EXECUTOR_PRIVATE_KEY` compromises the compatibility-derived EVM account or any unreconciled historical Moonbeam balance | Prefer the explicitly named EVM funding key, keep balances minimal, and reconcile historical Moonbeam addresses under RISK-020 |
...
- [x] Moonbeam runtime operations do not use `MOONBEAM_EXECUTOR_PRIVATE_KEY`; its legacy name remains a compatibility fallback for general EVM derivation — ✅ PASS
...
- [x] Server startup fails if `PENDULUM_FUNDING_SEED` or the currently required `MOONBEAM_EXECUTOR_PRIVATE_KEY` compatibility fallback is missing — ✅ PASS; no Moonbeam RPC follows from validation
