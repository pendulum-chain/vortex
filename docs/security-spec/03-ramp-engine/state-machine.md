...
11. **A user MUST have at most one validated nonterminal ramp and three provisional ramps** — Registration is provisional: the same user MAY register up to three distinct quotes in parallel while those ramps have `presigned_txs = NULL`. Registration locks the authenticated user's `profiles` row, expires old initial ramps, and counts those provisional rows before transaction preparation. The first `POST /v1/ramp/update` that submits a non-empty presigned subset which passes transaction validation reserves the active slot. After validation, `updateRamp` locks the ramp and the authenticated user's `profiles` row, checks for another nonterminal ramp with persisted presigned transactions, and persists the winner in the same transaction. Concurrent valid updates therefore serialize across API instances; the loser receives `409` with the winning ramp ID. Invalid presigned data cannot reserve the slot. Further valid partial updates to the winning ramp remain allowed, and its slot is released only when it reaches `complete`, `failed`, or `timedOut`.
...
13. **Only an authenticated owner MAY cancel an unsigned ramp** — `POST /v1/ramp/cancel` applies the standard ramp ownership guard, locks the ramp row, and permits only `initial → timedOut` while `presigned_txs IS NULL`. Repeating cancellation for an already `timedOut` ramp is idempotent. Once signatures have been persisted, payment instructions may have been exposed and cancellation returns `409`; the ramp remains resumable until expiry so a delayed payment is not silently abandoned. The dashboard and widget use a local back action after payment instructions are shown, rather than terminally cancelling the backend ramp.
...
| **Concurrent presigned updates** | Two provisionally registered ramps for one user submit valid presigned transactions at the same time and both attempt to become active | `updateRamp` validates each submission, then serializes on the user's profile row and atomically checks for another nonterminal ramp with `presigned_txs IS NOT NULL`; exactly one update commits and later contenders receive `409`. |
| **Unauthorized or late cancellation** | Another user cancels a ramp, or its owner attempts to cancel after money movement has started | Required dual authentication plus `assertRampOwnership` prevents cross-owner cancellation; the locked phase check permits only `initial` ramps and returns `409` after advancement. |
