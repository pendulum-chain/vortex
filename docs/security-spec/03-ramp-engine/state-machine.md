...
11. **A user MUST have at most one validated nonterminal ramp** — Registration is provisional: the same user MAY register distinct quotes in parallel while those ramps have `presigned_txs = NULL`. The first `POST /v1/ramp/update` that submits a non-empty presigned subset which passes transaction validation reserves the active slot. After validation, `updateRamp` locks the ramp and the authenticated user's `profiles` row, checks for another nonterminal ramp with persisted presigned transactions, and persists the winner in the same transaction. Concurrent valid updates therefore serialize across API instances; the loser receives `409` with the winning ramp ID. Invalid presigned data is rejected before the user lock/check and cannot reserve the slot. Further valid partial updates to the winning ramp remain allowed, and its slot is released only when it reaches `complete`, `failed`, or `timedOut`. Registration still changes `initial` ramps older than the 15-minute start window to `timedOut`.
...
13. **Only an authenticated owner MAY cancel an unstarted ramp** — `POST /v1/ramp/cancel` applies the standard ramp ownership guard, locks the ramp row, and permits only `initial → timedOut`. Repeating cancellation for an already `timedOut` ramp is idempotent. Any other phase returns `409`; cancellation MUST NOT interrupt a ramp after phase processing has advanced. The dashboard's **Cancel and go back** action and the widget's back action while awaiting onramp payment both await this endpoint before clearing local ramp state; a cancellation failure leaves the machine in its error path rather than silently abandoning the backend ramp.
