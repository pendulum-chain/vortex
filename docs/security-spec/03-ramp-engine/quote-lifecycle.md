...
- [x] Exchange rates used in quote calculation come from live sources: fiat forex from fastforex.io (with CoinGecko fallback and the configured short cache TTL), swap rates from Nabla DEX and SquidRouter API. **PASS** — verified: forex rates are resolved through `PriceFeedService`; swap rates come from Nabla/Squid.
