...
9. **Exchange rates MUST be sourced from authoritative sources** — Swap rates must come from the actual DEX (Nabla) or routing protocol (Squid). Fiat forex rates are sourced from fastforex.io and sanity-checked against CoinGecko's `usd-coin` fiat price; if fastforex is missing, unavailable, invalid, or outside the configured per-currency sanity band, CoinGecko is used as fallback. Cached forex rates must stay within the configured short TTL. If both fiat providers fail, quote/conversion paths must fail closed rather than reusing the input amount or proceeding with an unverified rate. Operators must treat the CoinGecko fallback/reference as a USDC-as-USD proxy, not as pure fiat FX, during USDC depeg conditions.
...
| **USDC-as-USD fallback divergence** | CoinGecko's `usd-coin` fiat price diverges from true USD fiat FX during a USDC depeg. Healthy FastForex quotes may fail the sanity band, or CoinGecko fallback may misprice quote/fee/subsidy math. | FastForex remains primary when inside the sanity band; provider failures fail closed if CoinGecko is also invalid. Operators should monitor spread warnings and quote availability during stablecoin stress. See `05-integrations/fastforex.md`. |
