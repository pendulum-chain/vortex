...
| **Unauthorized quote consumption** | Attacker binds someone else's quote to their own ramp | Quotes carrying an owner (`partner_id` or `user_id`) are bound to that owner; ownership is verified at ramp registration via `assertQuoteOwnership`. `pricing_partner_id` is not an ownership credential. `RampService.registerRamp` requires an effective user for **all** corridors and rejects anonymous quotes from being claimed by an authenticated caller (inv. 16), so an attacker cannot bind an anonymous quote to a user who did not create it. Fully-anonymous quotes (no `partner_id` and no `user_id`) are effectively unregistrable. |
