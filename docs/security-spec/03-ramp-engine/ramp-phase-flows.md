...
- `EurOfframpBase` binds the source token/network at quote time. The catalog matches any on-chain source symbol structurally; the flow input resolver rejects symbols unknown to the merged EVM token catalog (static config plus Squid-discovered tokens) at quote time, so persisted flows re-resolve at startup independently of live token discovery. Base USDC emits one user-wallet transfer; another Base token emits same-chain user-wallet Squid approve/swap; another EVM source emits cross-chain user-wallet Squid approve/swap. `fundEphemeral` verifies reported hashes against the issued payloads before platform funds move, using the same validation applied to BRL EVM offramps.
