...
`RampService` resolves only `quote.metadata.globals.request`, then invokes the selected flow's `register` and `prepareTxs` in sequence. Registration metadata refreshes are persisted in the ramp-registration transaction. Phase facts and response artifacts are flattened into the legacy top-level state/API fields required by active ramps, while `blockState` remains the phase-owned source of truth. No corridor selector or transaction route builder participates in registration dispatch.
...
10. **`destinationTransfer` MUST fail fast on a detectable nonce gap rather than retry-and-strand** — `blocks/phases/destination-transfer/execution.ts` reads the ephemeral's live nonce (`getTransactionCount`, `blockTag: "pending"`) and compares it to the presigned `destinationTransfer` nonce before broadcasting. If the presigned nonce is *ahead* of the live nonce the transfer can never mine, so the executor raises an `UnrecoverablePhaseError` for manual review instead of looping until the retry budget silently exhausts. Using `"pending"` rather than `"latest"` ensures the check accounts for mempool transactions. The live-nonce read is best-effort: an RPC failure or malformed transaction logs a warning and falls through to the normal balance-poll path.
...
- [EXISTING FINDING] **F-053 (narrowed)**: `blocks/phases/pendulum-to-assethub-xcm/execution.ts` trusts a persisted hash without status/arrival proof, and `blocks/phases/nabla-swap/execution.ts` has no durable broadcast marker. See `INHERITED-ISSUES.md`.
