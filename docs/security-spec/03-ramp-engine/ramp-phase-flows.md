...
- **Dormant replacement:** the previous Monerium Polygon→Moonbeam→XCM route remains removed. An uncataloged replacement factory now composes `moneriumOnrampMint` → `fundEphemeral` → `moneriumOnrampSelfTransfer` → `uniswapApprove` → `uniswapSwap` → `distributeFees` → `subsidizePostSwap` → `squidRouterSwap` → `squidRouterPay` → `finalSettlementSubsidy` → `destinationTransfer`. It converts EURe to native Polygon USDC through one pinned Uniswap V3 pool and then uses the regular EVM destination path. The issue executor fails closed, and no quote can select this topology. See `monerium.md`.
