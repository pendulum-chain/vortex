...
- **Charging** — the onramp deducts vortex/partner fees from the Alfredpay mint (bridge/transfer legs move mint − fees); the offramp deducts them from the bridged USD leg before pricing the Alfredpay payout (`OffRampAlfredpayDiscountEngine`). In both directions the fee residual is left on the Polygon ephemeral.
...
- **Subsidy interaction** — both Alfredpay discount engines subtract the charged fee from the subsidy's expected-output target, so a partner discount subsidy can never refill a fee that was just deducted (invariant 7).
...
- **Amount canonicalization** — every consumer of the charged fee total (fee transfers, settlement target, fallback refund, onramp swap sizing, offramp deduction) derives it from the same component-wise raw rounding (`computeFeeComponentRaws` / `getAlfredpayFeeTotalRaw`), and the offramp deduction applies the same 2-fiat-decimal rounding and price-feed conversion PER COMPONENT as the persisted fee metadata (aggregate rounding would diverge when both components carry sub-cent halves) — bridged amount = Alfredpay deposit + distributed fees in exact raw units (pinned by the reconciliation test in `mxn-offramp.scenario.test.ts`).
