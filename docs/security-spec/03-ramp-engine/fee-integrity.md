...
- **Idempotency** — each fee transfer's hash is persisted per nonce (`distributeFeeHashes`), so a phase retry never re-pays a recipient whose transfer already succeeded. Retries reconcile paid transfers BEFORE the balance precondition and require only the unpaid amount (decoded from the pending presigns), so a partial success cannot wedge the phase on a balance that can no longer cover the original total.
- **Amount canonicalization** — every consumer of the charged fee total (fee transfers, settlement target, fallback refund, onramp swap sizing, offramp deduction) derives it from the same component-wise raw rounding (`computeFeeComponentRaws` / `getAlfredpayFeeTotalRaw`), and the offramp deduction applies the same 2-fiat-decimal rounding and price-feed conversion as the persisted fee metadata — bridged amount = Alfredpay deposit + distributed fees in exact raw units (pinned by the reconciliation test in `mxn-offramp.scenario.test.ts`).
