...
- [ ] **Base Multicall3 split distribution cannot move ephemeral funds** — `createEvmFeeDistributionTransaction`'s partner-split path batches plain ERC-20 `transfer` calls through `Multicall3.aggregate3`; those execute with Multicall3 as `msg.sender` and would move the contract's (empty) balance, reverting the whole batch. Any Base ramp with a nonzero partner markup and a configured partner `payout_address_evm` would fail at `distributeFees`. The vortex-only path (direct transfer) is unaffected. Needs the same sequential-transfer approach the Alfredpay/Polygon builder uses.
