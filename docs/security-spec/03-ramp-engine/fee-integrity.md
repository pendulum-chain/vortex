...
- **Platform fees** (vortex, network, partner markup) are distributed during the `distributeFees` phase, which dispatches to a Substrate (Pendulum) or EVM (Base/Polygon, sequential ERC-20 transfers) implementation based on the ephemeral chain in use.
...
2. **EVM (Base)** — plain sequential USDC `transfer` transactions signed by the ephemeral at consecutive nonces, one per fee recipient (`addEvmFeeDistributionTransaction`): network+vortex fees to the vortex `payout_address_evm`, partner markup to the pricing partner's address. Recipient addresses come from `partner_pricing_configs.payout_address_evm`. When fees are nonzero, the builder requires the active `vortex` pricing config for the quote's ramp direction to have a non-NULL `payout_address_evm` (or the `DEFAULT_VORTEX_EVM_PAYOUT_ADDRESS` fallback) and fails registration otherwise; partner-markup recipients resolve through the quote's pricing partner (`pricing_partner_id ?? partner_id`); a nonzero markup with no partner `payout_address_evm` fails registration (fail closed — a charged fee must never lack a recipient). Multicall3 batching is deliberately NOT used (see item 3).
...
### Alfredpay corridors (USD/MXN/COP/ARS): Polygon fee distribution

3. **Polygon (Alfredpay corridors)** — the routes (`transactions/onramp/routes/alfredpay-to-evm.ts`, `transactions/offramp/routes/evm-to-alfredpay.ts`) prepare plain sequential USDT `transfer` transactions signed by the EVM ephemeral via `addAlfredpayFeeDistributionTransactions`: one for network+vortex fees to the vortex `payout_address_evm` (with the `DEFAULT_VORTEX_EVM_PAYOUT_ADDRESS` fallback), one for partner markup to the pricing partner's `payout_address_evm` (a nonzero markup with no partner payout address fails registration, mirroring Base). Multicall3 is deliberately NOT used: `aggregate3` executes calls with the Multicall3 contract as `msg.sender`, so a batched `transfer` cannot move the ephemeral's tokens.

Alfredpay fee flow properties:

- **Charging** — the onramp reserves vortex/partner fees separately from the canonical quoted bridge/transfer amount; the offramp deducts them from the bridged USD leg before pricing the Alfredpay payout (`OffRampAlfredpayDiscountEngine`). In both directions the fee residual is left on the Polygon ephemeral.
- **Ordering** — `distributeFees` is the LAST phase before `complete`, entered from `destinationTransfer` (onramp) or `alfredpayOfframpTransfer` (offramp) only when Polygon fee transfers were prepared at registration. Fees are collected only after the user-facing leg succeeded; failure paths (mint fallback / offramp refund fallback) never broadcast them, and the offramp refund fallback is sized deposit + fees so a failed ramp returns the user's full value.
- **Subsidy interaction** — the partner target discount specifies the user's final net rate. Therefore a subsidy may economically offset some or all charged fees (subject to `max_subsidy`), while the underlying fee amounts remain unchanged, fully reserved, and collected after the user-facing leg succeeds.
- **Solvency** — the offramp `finalSettlementSubsidy` expected amount is deposit + fees, so the ephemeral is topped up to cover the fee transfers; the distribute handler additionally polls the ephemeral's USDT balance before broadcasting.
- **Idempotency** — each fee transfer's hash is persisted per nonce (`distributeFeeHashes`), so a phase retry never re-pays a recipient whose transfer already succeeded. Retries reconcile paid transfers BEFORE the balance precondition and require only the unpaid amount (decoded from the pending presigns), so a partial success cannot wedge the phase on a balance that can no longer cover the original total. A persisted hash whose receipt shows a mined-but-REVERTED transfer fails the phase unrecoverably (its nonce is consumed, so rebroadcasting the primary presign could never succeed); recovery uses the validated same-call backups manually.
- **Amount canonicalization** — every consumer of the charged fee total (fee transfers, settlement target, fallback refund, onramp reserve target, offramp deduction) derives it from the same component-wise raw rounding (`computeFeeComponentRaws` / `getAlfredpayFeeTotalRaw`). Alfredpay offramp pricing freezes each rounded fiat component and its USD valuation before Discount; Fee metadata and distribution reuse that snapshot, preventing price-feed or fee-config drift. The onramp bridge/transfer amount comes from the quote's canonical `evmToEvm.inputAmountRaw`, while pre-swap settlement reserves that user amount plus the fee total.
...
7. **Subsidization MUST NOT bypass fee collection** — A target discount promises the user's final, net-of-fees rate, so its subsidy may economically offset platform fees. It MUST NOT reduce, skip, or redirect the separately reported fee components: those amounts remain fully reserved and are collected only after the user-facing leg succeeds, subject to the configured subsidy cap.
...
- [x] EVM branch of `distributeFees` uses plain sequential ERC-20 transfers (no Multicall3). **PASS (CHANGED 2026-07)** — see the "Base Multicall3 split distribution" item below for why the previous `aggregate3` batch was replaced.
...
- [x] **Alfredpay corridors charge AND collect vortex/partner fees** — previously the routes built no `distributeFees` transaction, so displayed vortex/partner components were uncollectable (onramp: deducted from the user and stranded on the ephemeral; offramp: displayed but never charged). **PASS (FIXED 2026-07)** — both directions now deduct the components from the user leg and collect the residual on Polygon via ephemeral-signed USDT transfers in a `distributeFees` phase that runs after the user is paid (see "Alfredpay corridors" above). Pinned by the "fee collection" end-to-end tests in `apps/api/src/tests/corridors/mxn-onramp.scenario.test.ts` and `mxn-offramp.scenario.test.ts` (payout-address balances asserted on the fake ledger).
- [x] **Base Multicall3 split distribution cannot move ephemeral funds** — the former partner-split path batched plain ERC-20 `transfer` calls through `Multicall3.aggregate3`; those execute with Multicall3 as `msg.sender` and would move the contract's (empty) balance, reverting the whole batch — any Base ramp with a nonzero partner markup and a configured partner `payout_address_evm` would have failed at `distributeFees`. **PASS (FIXED 2026-07)** — the Base builder now emits one plain ephemeral-signed transfer per fee recipient at consecutive nonces (shared implementation with the Alfredpay/Polygon builder), and the handler broadcasts them in nonce order with per-nonce hash idempotency (`distributeFeeHashes`; the legacy single `distributeFeeHash` is honored for in-flight ramps). Pinned by the "fee collection" test in `apps/api/src/tests/corridors/brl-offramp.scenario.test.ts`, which asserts a profile-priced partner's markup and the vortex fee each land on their payout address.
...
- [x] **Partner `payout_address_evm` NULL no longer drops markup silently**: quote creation rejects partner-markup routes when the partner lacks EVM payout config on every corridor that distributes fees on an EVM chain (`requiresEvmPartnerPayout` covers BRL-on-Base AND the Alfredpay USD/MXN/COP/ARS corridors), and the fee-distribution builder throws (failing registration) if the condition slips through — a charged markup can never be silently dropped. Pinned by the "markup partner without an EVM payout address" tests in the MXN corridor scenarios.
