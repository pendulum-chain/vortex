...
- **Idempotency** — each fee transfer's hash is persisted per nonce (`distributeFeeHashes`), so a phase retry never re-pays a recipient whose transfer already succeeded.
