...
10. **`finalSettlementSubsidy` MUST subsidize the gap to *actual bridge delivery*, not to the ephemeral's total balance** — The subsidy uses `delivered = actualBalance - settlementBaseline`, where the destination-specific baseline is snapshotted in `transactionPlan.settlementBaselines` before `squidRouterSwap` is broadcast. The snapshot is written only once. The balance poll waits for baseline + ≥90% of `expectedAmountRaw`, and the result is clamped to the observable on-chain shortfall. Computing from total balance without the baseline is unsafe because pre-existing dust can trigger an early, excessive top-up.
...
| Threat | Mitigation |
|---|---|
| **Final settlement subsidy cap bypass** — A missing or bypassed `throw` on the USD cap would allow a single ramp to drain the funding account's native token balance via an unbounded SquidRouter swap. | **Mitigated.** `final-settlement-subsidy/execution.ts` throws when `requiredNativeInUsd > MAX_FINAL_SETTLEMENT_SUBSIDY_USD`; keep this as a regression check because the blast radius is direct funding-key loss. |
...
| **Final settlement over-subsidy race** — `finalSettlementSubsidy` returns as soon as *any* destination-token balance appears and tops up before the bridge lands. | **Mitigated.** The flow records a destination-specific settlement baseline before bridge broadcast, waits for a ≥90% delivery delta, and clamps the subsidy to the on-chain shortfall. |
...
- [x] **`finalSettlementSubsidy` subsidizes against actual bridge delivery, not total balance.** **PASS** — `SquidRouterSwapExecutor` writes the immutable destination-specific baseline under `transactionPlan.settlementBaselines`; `FinalSettlementSubsidyExecutor` waits for the delivery delta and uses `calculateSettlementSubsidyRaw`, which clamps to the on-chain shortfall. Regression coverage lives in `phases/blocks/__tests__/settlement.test.ts`.
