...
- [x] **EVM subsidy handlers (`subsidize-pre-swap-evm-handler.ts`, `subsidize-post-swap-evm-handler.ts`) enforce a USD cap** via `MAX_EVM_SWAP_SUBSIDY_QUOTE_FRACTION`, and over-cap subsidies remain `UnrecoverablePhaseError` instead of being wrapped as recoverable retries.
