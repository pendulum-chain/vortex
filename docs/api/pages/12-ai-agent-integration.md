...
Resolve delegated authority from live child membership, not from owner identity alone. List/read lifecycle responses identify `actor` and decorate each child with `membership: { role, isOwner }` and its immutable owner's `policy`; creation remains undecorated. `read_only` includes quote creation but never mutations, even through a secret. Selected-child provider/KYC mutations require a `manager` member's own secret; ramp register/update/start also require that secret, with no bearer drain exception. Direct child credentials cannot select children, import BR share tokens, or administer membership. The nine membership/invitation routes are Supabase bearer-only and reject API-key headers; invitee acceptance has no request email or selector and requires the current verified Supabase email plus explicit acceptance. Member removal does not revoke shared child secrets. Follow the capability matrix and invitation examples linked above rather than treating a bearer as interchangeable with a secret.

Both list and detail expose `actor: { profileId, canProvisionManagedProfiles, hasMemberships }`. Use these flags, not a `200` or the current page length, for access discovery: the default list can be empty with both flags false, and `hasMemberships` counts eligible active children independently of pagination/status. Both `status=deleted` and `status=all` require the actor's own active manager configuration and include only its owned children. Detail bootstrap requires an exactly matching selector and stored membership history before returning membership-invalid; never-member existing and unknown children both return masked `404`. Retained deleted-child detail is owner-only without a selector. Bearer and member-secret callers follow the same read rules. Keep the shipped provider-mutation code `MANAGED_PROFILE_REQUIRES_API_CREDENTIAL`.
