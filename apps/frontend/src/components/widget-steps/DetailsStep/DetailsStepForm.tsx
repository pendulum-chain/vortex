...
export const DetailsStepForm = ({ isBrazilLanding, signingState, className }: DetailsStepFormProps) => {
  const { shouldDisplay: signingBoxVisible, progress } = signingState;

  return (
    <>
      <div className={`mt-8 grid flex-grow gap-3 ${className || ""}`}>{isBrazilLanding && <AveniaFormStep />}</div>
