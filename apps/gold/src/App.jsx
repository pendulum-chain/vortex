...
import { ArrowLeft, ArrowRight, Bank, CaretDown, Check, CheckCircle, Clock, Copy, EnvelopeSimple, Eye, EyeSlash, Fingerprint, Info, LockKey, SealCheck, ShieldCheck, SignOut, TrendUp, Wallet, WarningCircle, X } from "@phosphor-icons/react";
...
function Login({ onClose, onLogin, loading }) {
  ...
  return <Modal title="Seu ouro começa aqui." description="Entre sem senha com uma conta que você já usa. Uma carteira segura será criada automaticamente para você." onClose={onClose}><div className="login-trust"><span><Fingerprint size={21} /> Você confirma cada movimentação</span><span><LockKey size={21} /> Seu ouro fica na sua carteira</span></div><div className="login-methods"><button className="button button--google" type="button" onClick={() => onLogin("google")} disabled={Boolean(loading)}><span className="google-mark" aria-hidden="true">G</span>{loading === "google" ? "Entrando…" : "Continuar com Google"}</button><button className="login-other-trigger" type="button" onClick={() => setShowOtherMethods((current) => !current)} aria-expanded={showOtherMethods}><span>Outras formas de entrar</span><CaretDown size={16} className={showOtherMethods ? "open" : ""} /></button>{showOtherMethods && <button className="button button--email" type="button" onClick={() => onLogin("email")} disabled={Boolean(loading)}><EnvelopeSimple size={22} />{loading === "email" ? "Entrando…" : "Continuar com e-mail"}</button>}</div><p className="legal-note">Ao continuar, você aceita os Termos de Uso e a Política de Privacidade.</p></Modal>;
}
