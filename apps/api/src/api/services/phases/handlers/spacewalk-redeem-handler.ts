...
export class SpacewalkRedeemPhaseHandler extends BasePhaseHandler {
  ...
  protected async executePhase(state: RampState): Promise<RampState> {
    ...
    const outputAmountUnits = quote.metadata.pendulumToStellar?.outputAmountDecimal.toString();
    const outputAmountRaw = quote.metadata.pendulumToStellar?.outputAmountRaw;
    ...
    try {
      ...
      const accountJson = accountData.toJSON() as { nonce?: number } | null;
      const currentEphemeralAccountNonce = accountJson?.nonce;
