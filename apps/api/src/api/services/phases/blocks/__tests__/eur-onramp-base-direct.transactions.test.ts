...
describe("EUR onramp Base direct transactions", () => {
  it("prepares the nonce-zero transfer with no cleanup and preserves owned state", async () => {
    ...
    expect(prepared.unsignedTxs.map(tx => [tx.phase, tx.network, tx.signer, tx.nonce])).toEqual([
      ["destinationTransfer", Networks.Base, EPHEMERAL, 0]
    ]);
    expect(prepared.unsignedTxs[0].txData).toMatchObject({ gas: "100000", value: "0" });
