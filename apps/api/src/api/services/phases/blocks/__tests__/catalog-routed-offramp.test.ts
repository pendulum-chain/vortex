...
describe("SELL flow catalog with routed (Squid-discovered) source tokens", () => {
  ...
  it("maps a routed EVM source token to the BRL and Alfredpay offramp flows without live token discovery", () => {
  ...
  it("keeps EUR SELL unavailable for new quotes even for a routed source token", () => {
    // EurOfframpBase is retained for persisted Mykobo recovery only (newQuotes: false).
    expect(() => resolveBlockFlow(sellRequest("PAXG", FiatToken.EURC))).toThrow(/No block flow mapped/);
  });
