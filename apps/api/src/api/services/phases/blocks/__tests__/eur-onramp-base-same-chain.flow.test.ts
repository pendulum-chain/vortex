...
const EUR_ONRAMP_BASE_SAME_CHAIN: RampPhase[] = ["initial", ...COMMON_PHASES, "destinationTransfer", "complete"];
const EUR_ONRAMP_BASE_SAME_CHAIN_SWAP: RampPhase[] = [
  "initial",
  ...COMMON_PHASES,
  "squidRouterSwap",
  "destinationTransfer",
...
describe("EUR Base same-chain block flows", () => {
  ...
  for (const outputCurrency of [EvmToken.USDC, ...ROUTED_BASE_OUTPUTS]) {
    it(`simulates EUR to Base ${outputCurrency} with provider fees and phase metadata`, async () => {
