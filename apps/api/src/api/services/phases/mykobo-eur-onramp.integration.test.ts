...
mock.module("./blocks/core/nabla", () => {
  return {
    calculateNablaSwapOutputEvm: async (request: {
      ...
      const decimalOut = new Big(request.inputAmountForSwap).times("1.05");
...
RampState.create = mock(async (data: RampStateCreationAttributes): Promise<RampState> => {
  rampState = {
    ...data,
    createdAt: new Date(),
    id: data.id || "test-mykobo-onramp-ramp-id",
...
QuoteTicket.create = mock(async (data: QuoteTicketCreationAttributes): Promise<QuoteTicket> => {
  quoteTicket = {
    ...data,
    createdAt: new Date(),
    id: data.id || "test-mykobo-onramp-quote-id",
    update: async function (
...
// Opt-in via RUN_LIVE_TESTS=1 (see docs/testing.md).
describe.skipIf(!process.env.RUN_LIVE_TESTS)("Mykobo EUR onramp contract test (real sandbox, no on-chain submission)", () => {
  ...
  it("hits Mykobo /fees and returns a numeric total for DEPOSIT", async () => {
  ...
  it("creates a Mykobo DEPOSIT intent and returns IBAN instructions", async () => {
    const mykobo = MykoboApiService.getInstance();
    let intent;
    try {
    ...
    if (!intent.instructions || !("iban" in intent.instructions)) {
      throw new Error("Expected deposit instructions with `iban` field");
    }
    expect(intent.instructions.iban).toMatch(IBAN_REGEX);
    expect(intent.instructions.bank_account_name).toBeTruthy();
