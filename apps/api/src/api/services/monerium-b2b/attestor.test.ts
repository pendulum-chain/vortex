...
import { Address, encodePacked, hashMessage, Hex, hexToBigInt, hexToNumber, keccak256, recoverAddress, slice, stringToBytes } from "viem";
...
describe("monerium-b2b attestor", () => {
  it("derives LINK_HASH_191 and LINK_HASH_RAW exactly as the forwarder immutables", () => {
    ...
    expect(linkMessageHash("raw")).toBe(keccak256(stringToBytes(LINK_MESSAGE)));
    expect(linkMessageHash("eip191")).toBe(keccak256(stringToBytes(`\x19Ethereum Signed Message:\n45${LINK_MESSAGE}`)));
    // Independent derivation via viem's EIP-191 implementation.
    expect(linkMessageHash()).toBe(hashMessage(LINK_MESSAGE));
  });

  it("binds the signature to the forwarder address exactly like isValidSignature", async () => {
    const attestation = await signLinkAttestation(FORWARDER);
    // bound = keccak256(abi.encodePacked(address(this), hash)) — the contract-side recomputation.
    const expectedBound = keccak256(encodePacked(["address", "bytes32"], [FORWARDER, hashMessage(LINK_MESSAGE)]));
  ...
  it("emits a 65-byte low-s signature with v in 27/28 for both hash variants", async () => {
    for (const variant of ["eip191", "raw"] as const) {
      const { boundHash, linkHash, signature } = await signLinkAttestation(FORWARDER, variant);
      ...
      expect(boundHash).toBe(attestationBoundHash(FORWARDER, linkHash));
      expect(await recoverAddress({ hash: boundHash, signature })).toBe(privateKeyToAccount(TEST_KEY).address);
    }
  });

  it("does not verify against a different forwarder address", async () => {
    const { signature } = await signLinkAttestation(FORWARDER);
    const otherBound = attestationBoundHash("0x2222222222222222222222222222222222222222", linkMessageHash());
    expect(await recoverAddress({ hash: otherBound, signature })).not.toBe(privateKeyToAccount(TEST_KEY).address);
  });

  it("refuses to sign when the attestor key is not configured", async () => {
    config.moneriumB2b.attestorPrivateKey = undefined;
    try {
      await expect(signLinkAttestation(FORWARDER)).rejects.toThrow("MONERIUM_B2B_ATTESTOR_PRIVATE_KEY");
    } finally {
      config.moneriumB2b.attestorPrivateKey = TEST_KEY;
    }
  });
