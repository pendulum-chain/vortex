...
export const PendulumOfframpNablaSwap: Phase<
...
> = {
  ...
  name: "PendulumOfframpNablaSwap(USDC->BRL)",
  ...
  async prepareTxs(ctx) {
    ...
    const input = PENDULUM_USDC_ASSETHUB;
    const output = getPendulumDetails(FiatToken.BRL);
    ...
    return {
      intents: [
        ...
        { lane: "main", network: Networks.Pendulum, phase: "nablaSwap", signer: account.address, txData: swap.transaction },
        {
          lane: "cleanup",
          network: Networks.Pendulum,
  ...
  async simulate(input, ctx) {
    const inputDetails = PENDULUM_USDC_ASSETHUB;
    const outputDetails = getPendulumDetails(FiatToken.BRL);
