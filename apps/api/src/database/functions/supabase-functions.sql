...
CREATE OR REPLACE FUNCTION get_monthly_volumes(year_param INTEGER)
RETURNS TABLE(month TEXT, volume NUMERIC)
LANGUAGE SQL
AS $$
  SELECT
  ...
  WHERE rs.current_phase = 'complete'
    AND date_trunc('year', rs.created_at) = (year_param || '-01-01')::date
  GROUP BY 1
  ORDER BY 1;
$$;
