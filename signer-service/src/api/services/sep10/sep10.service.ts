...
export const signSep10Challenge = async (
  ...
  const { signingKey: anchorSigningKey } = (await fetchTomlValues(
    outTokenConfig.tomlFileUrl
  )) as TomlValues;
  const { homeDomain, clientDomainEnabled, memoEnabled } = outTokenConfig;
  const transactionSigned = TransactionBuilder.fromXDR(
    challengeXDR,
    NETWORK_PASSPHRASE
  );
  ...
  const masterClientSignature =
    memo !== null && memoEnabled
      ? transactionSigned.getKeypairSignature(masterStellarKeypair)
      : undefined;
