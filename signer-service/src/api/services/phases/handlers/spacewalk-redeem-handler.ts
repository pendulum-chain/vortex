...
export class SpacewalkRedeemPhaseHandler extends BasePhaseHandler {
  ...
  protected async executePhase(state: RampState): Promise<RampState> {
    ...
    try {
      ...
      if (currentEphemeralAccountNonce !== undefined && currentEphemeralAccountNonce > executeSpacewalkNonce) {
        await this.waitForOutputTokensToArriveOnStellar(outputAmountBeforeFees.units, stellarTarget.stellarTargetAccountId, stellarTarget.stellarTokenDetails.stellarAsset.code.string );
      ...
      console.log(
        `Requesting redeem of ${outputAmountBeforeFees.units} tokens for vault ${vaultService.vaultId}`,
      );
      ...
      console.log(
        `Successfully posed redeem request ${redeemRequestEvent.redeemId} for vault ${vaultService.vaultId}`,
      );
  ...
  private async waitForOutputTokensToArriveOnStellar(outputAmountUnits: string, targetAccount: string, stellarAssetCode: string): Promise<void> {
