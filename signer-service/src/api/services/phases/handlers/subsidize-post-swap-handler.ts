...
export class SubsidizePostSwapPhaseHandler extends BasePhaseHandler {
  ...
  protected async executePhase(state: RampState): Promise<RampState> {
    ...
    try {
        const balanceResponse = await pendulumNode.api.query.tokens.accounts(pendulumEphemeralAddress, outputTokenPendulumDetails.pendulumCurrencyId);
        ...
        if (requiredAmount.gt(Big(0))) {
            ...
            await pendulumNode.api.tx.tokens.transfer(pendulumEphemeralAddress, outputTokenPendulumDetails.pendulumCurrencyId, requiredAmount.toFixed(0, 0)).signAndSend(fundingAccountKeypair);
        }
