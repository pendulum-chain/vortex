  ...
  protected async executePhase(state: RampState): Promise<RampState> {
    ...
    try {
        ...
        if (requiredAmount.gt(Big(0))) {
            ...
            await pendulumNode.api.tx.tokens.transfer(pendulumEphemeralAddress, inputTokenPendulumDetails.pendulumCurrencyId, requiredAmount.toFixed(0, 0)).signAndSend(fundingAccountKeypair);
        }
