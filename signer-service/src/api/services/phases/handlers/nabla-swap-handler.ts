...
export class NablaSwapPhaseHandler extends BasePhaseHandler {
  ...
  protected async executePhase(state: RampState): Promise<RampState> {
    ...
    if (!nablaSoftMinimumOutputRaw || !pendulumEphemeralAddress || !pendulumAmountRaw || !inputTokenPendulumDetails || !outputTokenPendulumDetails) {
    ...
    try {
        ...
        const response = await readMessage({
            ...
            messageArguments: [
                pendulumAmountRaw,
                [(inputTokenPendulumDetails as PendulumDetails).pendulumErc20WrapperAddress, (outputTokenPendulumDetails as PendulumDetails).pendulumErc20WrapperAddress],
            ],
