...
export async function getTokenOutAmount(params: {
  ...
  const amountIn = multiplyByPowerOfTen(amountBig, fromTokenDecimals).toFixed(
    0,
    0
  );
  ...
  const result = await contractRead<TokenOutData>({
    ...
    args: [
      amountIn,
      [
        inputTokenDetails.pendulumErc20WrapperAddress,
        outputTokenDetails.pendulumErc20WrapperAddress,
      ],
    ],
    api,
    noWalletAddressRequired: true,
    parseSuccessOutput: (data: any) => {
      const preciseQuotedAmountOut = parseContractBalanceResponse(
        outputTokenDetails.pendulumDecimals,
        data[0]
      );
      const swapFee = parseContractBalanceResponse(
        outputTokenDetails.pendulumDecimals,
        data[1]
      );
      return {
        ...
        roundedDownQuotedAmountOut:
          preciseQuotedAmountOut.preciseBigDecimal.round(2, 0),
