...
export const validatePriceInput: RequestHandler<
  {},
  unknown,
  unknown,
  PriceQuery
...
export const validateChangeOpInput: RequestHandler = (req, res, next) => {
  const { accountId, sequence, paymentData, maxTime, assetCode, baseFee } =
    req.body as ChangeOpBody;
...
const validateRequestBodyValuesForTransactionStore =
  (): RequestHandler => (req, res, next) => {
    ...
    const requiredRequestBodyKeys =
      FLOW_HEADERS[flowType as keyof typeof FLOW_HEADERS];
...
export const validateStorageInput =
  validateRequestBodyValuesForTransactionStore();
export const validateEmailInput = validateRequestBodyValues(
  EMAIL_SHEET_HEADER_VALUES
);
export const validateRatingInput = validateRequestBodyValues(
  RATING_SHEET_HEADER_VALUES
);
export const validateExecuteXCM = validateRequestBodyValues(["id", "payload"]);

export const validatePreSwapSubsidizationInput: RequestHandler = (
  req,
  res,
  next
) => {
...
export const validatePostSwapSubsidizationInput: RequestHandler = (
  req,
  res,
  next
) => {
...
export const validateSep10Input: RequestHandler = (req, res, next) => {
  ...
  if (!outToken) {
    res
      .status(400)
      .json({ error: "Missing offramp token identifier: outToken" });
    return;
  }
  ...
  if (!clientPublicKey) {
    res
      .status(400)
      .json({ error: "Missing Stellar ephemeral public key: clientPublicKey" });
    return;
  }
  next();
};
...
export const validateBrlaTriggerOfframpInput: RequestHandler = (
  req,
  res,
  next
) => {
  const { taxId, pixKey, amount, receiverTaxId } =
    req.body as TriggerOfframpRequest;
...
export const validataSubaccountCreation: RequestHandler = (req, res, next) => {
  ...
  if (taxIdType !== "CPF" && taxIdType !== "CNPJ") {
    res
      .status(400)
      .json({ error: "taxIdType parameter must be either CPF or CNPJ" });
    return;
  }
