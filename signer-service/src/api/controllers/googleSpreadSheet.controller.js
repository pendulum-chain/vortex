...
exports.storeDataInGoogleSpreadsheet = async (req, res, spreadsheetId, SHEET_HEADER_VALUES) => {
  try {
    // We expect the data to be an object that matches our schema
    const data = req.body;

    // Try dumping transactions to spreadsheet
    const sheet = await initGoogleSpreadsheet(spreadsheetId, spreadsheet.googleCredentials).then((doc) => getOrCreateSheet(doc, SHEET_HEADER_VALUES));
