...
import {
  ...
  getFundingData,
} from "../services/pendulum/pendulum.service";
import {
  ChainDecimals,
  multiplyByPowerOfTen,
  nativeToDecimal,
} from "../services/pendulum/helpers";
import { SlackNotifier } from "../services/slack.service";
import {
  ApiManager,
  SubstrateApiNetwork,
} from "../services/pendulum/apiManager";
...
export const fundEphemeralAccountController = async (
  ...
  try {
    const result = await fundEphemeralAccount(
      networkName,
      ephemeralAddress,
      Boolean(requiresGlmr)
    );
    if (result) {
...
export const sendStatusWithPk = async (): Promise<StatusResponse> => {
  ...
  const { fundingAccountKeypair } = getFundingData(
    apiData.ss58Format,
    apiData.decimals
  );
  const { data: balance } = (await apiData.api.query.system.account(
  ...
  await Promise.all(
    Object.entries(TOKEN_CONFIG).map(
      async ([token, tokenConfig]: [
        ...
        const maximumSubsidyAmountRaw = Big(
          tokenConfig.maximumSubsidyAmountRaw
        );
        const remainingMaxSubsidiesAvailable = tokenBalance.div(
          maximumSubsidyAmountRaw
        );

        if (
          remainingMaxSubsidiesAvailable.lt(SUBSIDY_MINIMUM_RATIO_FUND_UNITS)
        ) {
          isTokensSufficient = false;
          console.log(`Token ${token} balance is insufficient.`);

          const tokenDecimals =
            "decimals" in tokenConfig ? tokenConfig.decimals : ChainDecimals;
          slackNotifier.sendMessage({
            text: `Current balance of funding account is ${nativeToDecimal(
              tokenBalance,
              tokenDecimals
            ).toString()} ${token} please charge the account ${
              fundingAccountKeypair.address
            }.`,
          });
        }
  ...
  const minimumBalanceFundingAccount = multiplyByPowerOfTen(
    Big(PENDULUM_FUNDING_AMOUNT_UNITS),
    apiData.decimals
  );
  ...
  if (
    nativeBalance.lt(minimumBalanceFundingAccount) ||
    glmrBalance.lt(minimumGlmrBalanceFundingAccount)
  ) {
    slackNotifier.sendMessage({
      text: `Current balance of funding account is ${nativeToDecimal(
        nativeBalance
        ...
        glmrBalance,
        TOKEN_CONFIG.glmr.decimals
      ).toString()} GLMR, please charge the account ${
        fundingAccountKeypair.address
      }.`,
    });
  }
