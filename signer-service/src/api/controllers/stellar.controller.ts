...
const FUNDING_PUBLIC_KEY = FUNDING_SECRET
  ? Keypair.fromSecret(FUNDING_SECRET).publicKey()
  : "";
...
export const getSep10MasterPKHandler = async (
  ...
  try {
    ...
    const masterSep10Public =
      Keypair.fromSecret(SEP10_MASTER_SECRET).publicKey();
...
export async function sendStatusWithPk(): Promise<StatusResult> {
  ...
  try {
    ...
    if (
      !stellarBalance ||
      Number(stellarBalance.balance) < Number(STELLAR_FUNDING_AMOUNT_UNITS)
    ) {
